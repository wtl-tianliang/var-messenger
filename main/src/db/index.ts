...
export async function initDB() {
  ...
  await db.exec(
    "CREATE TABLE login (id INTEGER PRIMARY KEY AUTOINCREMENT, smtp varchar(255), smtp_port int, username varchar(255), password varchar(255), use_secure int, is_drop int)"
  );
...
export async function insertLogin(
  ...
  const result = await db.run(
    "INSERT INTO login (smtp, smtp_port, username, password, use_secure, is_drop) VALUES (:smtp, :smtp_port, :username, :password, :use_secure, :is_drop)",
    row
  );
  await db.close();
 ...
 */
export async function getLogin(offset = 0, limit = 0) {
  const db = await openDB();
  let sql = "";
  if (limit && limit > 0) {
